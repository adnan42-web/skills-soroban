...
3. **Deep pass (≤3 sentences per vector).** Only for surviving vectors. For each: reason in ≤3 sentences about whether and how the pattern applies — consider alternate manifestations, not just the literal construct named. If you cannot confirm the attack path in 3 sentences, drop it. If no match or FP conditions fully apply → move on (never reconsider). If match → apply the FP gate from `judging.md` immediately (three checks). If any check fails → drop and move on. Only if all three pass → confirm attack path in ≤3 intermediate calls, apply score deductions, and format the finding.
