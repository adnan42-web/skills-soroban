...
3. **Deep pass.** Only for surviving vectors. For each: reason about whether and how the pattern applies — consider alternate manifestations, not just the literal construct named. If no match or FP conditions fully apply → move on (never reconsider). If match → apply the FP gate from `judging.md` immediately (three checks). If any check fails → drop and move on. Only if all three pass → confirm attack path in ≤3 intermediate calls, apply score deductions, and format the finding.
